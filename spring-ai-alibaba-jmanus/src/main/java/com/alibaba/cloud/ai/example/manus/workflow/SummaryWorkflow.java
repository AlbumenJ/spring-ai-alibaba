...
public class SummaryWorkflow implements ISummaryWorkflow {
	 ...
	 * @param planId Caller's plan ID, ensuring subprocess can find corresponding
	 * directory
	 * @param fileName File name
	 * @param content File content
	 * @param queryKey Query keyword
	 * @param thinkActRecordId Think-act record ID for subprocess execution tracking
	...
	/**
	 * Build MapReduce-based summary execution plan
	...
	private MapReduceExecutionPlan buildSummaryExecutionPlan(String parentPlanId, String fileName, String content,
		...
		try {
			// Use caller-provided planId instead of generating new one
	...
	private CompletableFuture<String> executeMapReducePlanWithContext(String rootPlanId,
		...
		return CompletableFuture.supplyAsync(() -> {
			try {
				...
				return "getContent execution successful, execution result log: " + lastStep.getResult();
			}
			catch (Exception e) {
